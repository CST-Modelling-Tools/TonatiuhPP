...
SO_NODE_SOURCE(ShapeFunctionN)


void ShapeFunctionN::initClass()
{
    SO_NODE_INIT_CLASS(ShapeFunctionN, ShapeRT, "ShapeRT");
}

ShapeFunctionN::ShapeFunctionN()
{
    SO_NODE_CONSTRUCTOR(ShapeFunctionN);
...
bool ShapeFunctionN::intersect(const Ray& ray, double* tHit, DifferentialGeometry* dg, ProfileRT* profile) const
{
    // r0_z + d_z*t = 0
...
void ShapeFunctionN::updateShapeGL(TShapeKit* parent)
{
    makeQuadMesh(parent, QSize(2, 2));
}

void ShapeFunctionN::onSensor(void* data, SoSensor*)
{
    ShapeFunctionN* shape = (ShapeFunctionN*) data;
