...
SO_NODE_SOURCE(ShapeNormalMap)


void ShapeNormalMap::initClass()
{
    SO_NODE_INIT_CLASS(ShapeNormalMap, ShapeRT, "ShapeRT");
}

ShapeNormalMap::ShapeNormalMap()
{
    SO_NODE_CONSTRUCTOR(ShapeNormalMap);

    SO_NODE_ADD_FIELD( fX, (1.) );
    SO_NODE_ADD_FIELD( fY, (1.) );
}

vec3d ShapeNormalMap::getPoint(double u, double v) const
{
    return vec3d(
        u,
        v,
...
vec3d ShapeNormalMap::getNormal(double u, double v) const
{
    return vec3d(
        -u/fX.getValue(),
        -v/fY.getValue(),
...
Box3D ShapeNormalMap::getBox(ProfileRT* profile) const
{  
...
// x^2*gx + y^2*gy = 4z
// r = r0 + d*t
bool ShapeNormalMap::intersect(const Ray& ray, double* tHit, DifferentialGeometry* dg, ProfileRT* profile) const
{
    ...
    for (int i = 0; i < 2; ++i)
    {
        ...
        if (tHit == 0 && dg == 0)
            ...
            gcf::SevereError("ShapeNormalMap::intersect");
...
void ShapeNormalMap::updateShapeGL(TShapeKit* parent)
{
