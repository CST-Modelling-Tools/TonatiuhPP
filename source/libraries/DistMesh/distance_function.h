...
namespace distanceFunction {
    ...
    TONATIUH_LIBRARIES Functional rectangle(Eigen::Ref<Eigen::ArrayXXd const> const rectangle);

    // creates distance function for elliptical domains
    // Note: not a real distance function but a level function,
    // which is sufficient
    TONATIUH_LIBRARIES Functional elliptical(Eigen::Ref<Eigen::ArrayXd const> const radii=Eigen::ArrayXd(),
        Eigen::Ref<Eigen::ArrayXd const> const midpoint=Eigen::ArrayXd());

    // creates the true distance function for circular domains
    TONATIUH_LIBRARIES Functional circular(double const radius=1.0,
        Eigen::Ref<Eigen::ArrayXd const> const midpoint=Eigen::ArrayXd());
